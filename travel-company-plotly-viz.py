...
def create_visualization():
    ...
    # Create custom HTML with animation controls
    custom_html = """
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="control-panel">
        ...
        <div class="timeline" id="timeline">
            ...
            <!-- Timeline years will be added dynamically -->
        </div>
    ...
    <script>
        window.addEventListener('load', async function() {{
            ...
            let historicalMaxRevenue = Math.max(...allQuartersData[0].revenues);
            ...
            // Set up timeline
            const startYear = Math.floor(allQuartersData[0].year);
            const endYear = Math.ceil(allQuartersData[allQuartersData.length-1].year);
            
            for (let year = startYear; year <= endYear; year++) {{
                if (year % 2 === 0 || year === startYear || year === endYear) {{
                    const yearPercent = (year - startYear) / (endYear - startYear) * 100;
                    const yearMarker = document.createElement('div');
                    yearMarker.className = 'timeline-year';
                    yearMarker.style.left = yearPercent + '%';
                    yearMarker.textContent = year;
                    timelineElem.appendChild(yearMarker);
                }}
            }}
            ...
            async function updateChart(index) {{
                ...
                // Update slider, quarter display, and timeline
                quarterSlider.value = index;
                currentQuarterDisplay.textContent = currentData.quarter;
                
                const timelinePosition = (currentData.year - startYear) / (endYear - startYear) * 100;
                timelineMarker.style.left = timelinePosition + '%';
                
                // Calculate max revenue for this frame
                let frameMaxRevenue = Math.max(...currentData.revenues);
                historicalMaxRevenue = Math.max(historicalMaxRevenue, frameMaxRevenue);
                let xAxisMax = historicalMaxRevenue * 1.5;
                
                // Update logos with proper positioning
                const globalXOffset = historicalMaxRevenue * 1.05;
                const fixedLogoWidth = historicalMaxRevenue * 0.2;
                
                // Update chart data
                await Plotly.update(chartDiv, {{
            ...
            async function playAnimation() {{
                ...
                async function advance() {{
                    ...
                    await updateChart(currentQuarterIndex);
                    
                    if (isPlaying) {{
                        playInterval = setTimeout(advance, {animation_duration});
                    }}
                }}
    ...
    """.format(
        ...
        max_quarters=len(all_quarters_data) - 1,
