...
desired_width_in_inches = 0.006  # Increased logo size
...
def update(frame, preview=False):
    ...
    # Filter data for companies with revenue > 0 (except OWW)
    ...
    # Create horizontal bar chart with increased height
    bar_height = 0.8  # Increased bar height
    bars = current_ax.barh(
        y_positions,
    ...
    # Set y-axis limits with more space for logos
    current_ax.set_ylim(-0.5, max_bars - 0.5)

    # Add company names as y-tick labels with larger font size
    current_ax.set_yticks(y_positions)
    current_ax.set_yticklabels(top_companies['Company'], fontproperties=open_sans_font, fontsize=14)  # Increased font size

    # Invert y-axis to have highest revenue at the top
    current_ax.invert_yaxis()

    # Add revenue labels and logos
    for i, (bar, revenue, company) in enumerate(zip(bars, top_companies['Revenue'], top_companies['Company'])):
        width = bar.get_width()
        y_pos = bar.get_y() + bar.get_height() / 2

        # Add revenue value with larger font
        if width > 0:
            label = current_ax.text(
                width * 1.02,  # Slightly offset from the end of the bar
                y_pos,
                f'{width:,.0f}',
                va='center',
                ha='left',
        ...
        # Add logo
        if company in logos:
            # Choose the correct logo based on time period
            if company == 'PCLN':
                if 2014.25 <= frame < 2018.08:
                ...
                if frame < 2015.42 and frame > 2004:
                ...
                image_path = logos[company]

            # Calculate logo size based on revenue with increased base size
            base_zoom = get_zoom_factor(image_path, desired_width_in_inches, current_ax)
            relative_size = (revenue / max_revenue_value) ** 0.5
            zoom_factor = base_zoom * (1.0 + 0.5 * relative_size)  # Increased base size and scaling factor

            # Position logo after the revenue value with more space
            imagebox = OffsetImage(image_path, zoom=zoom_factor)
            ab = AnnotationBbox(
                imagebox,
                (width * 1.20, y_pos),  # Increased spacing after revenue value
                frameon=False,
    ...
    # Adjust aesthetics
    current_ax.set_xlim(0, max_revenue_value * 1.4)  # Extended x-axis more for larger logos
    ...
    # Increased font size for x-axis label
    current_ax.set_xlabel('Revenue TTM (in Millions)', fontsize=16, fontproperties=open_sans_font, labelpad=33)

    # Add quarter/year text in top-left corner with larger font
    current_ax.text(
        0.02, 0.98,
        get_quarter_year(frame),
